...
def fake_enquote(matchobj):
    escaped_item = matchobj.group(0).replace('"', '\\"').replace("=", "!!!")
    return f'"!!{escaped_item}!!"'

def fake_dequote(string):
...
for filename in decision_files:
    with open(f"{decision_folder}/{filename}", "r") as file:
        ...
        string = regex.sub(r"\b[a-zA-Z_]*color\b ?= ?\K.*", fake_enquote, string)
        ...
        if "country_decisions" not in data:
            continue
...
class MyEncoder(json.JSONEncoder):
    def encode_dict_item(self, key, value, depth):
        if isinstance(value, list):
            ...
            return """{0}{1}{2}{3}""".format(self.indent * depth, key, self.key_separator,
                                             MyEncoder.encode(self, value, key, depth=depth + 1))
    ...
    def encode(self, obj, key=None, depth=0):
        ...
        if isinstance(obj, list):
            items = item_separator.join(
                ["""{0}{1}{2}{3}""".format(indent, key, key_separator, MyEncoder.encode(self, item, depth=depth + 1))
                 for item in obj])
            return items
...
updated_string = updated_string.replace("@", ":").replace("\\\"", "~").replace("\"", "").replace("~", "\"")
updated_string = fake_dequote(updated_string)
